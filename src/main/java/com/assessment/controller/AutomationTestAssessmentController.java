...
public class AutomationTestAssessmentController {
	...
	public ModelAndView navigationQuestions1a(Model model, ModelAndView view, HttpSession httpSession) {
		...
		/*System.out.println("After 1st questions : GET");
		System.out.println(new GsonBuilder().setPrettyPrinting().create().toJson(totalInformation));
		*/
...
//		System.out.println(new GsonBuilder().setPrettyPrinting().create().toJson(totalInformation));
	...
	public ModelAndView submittedQuestions(Model model, ModelAndView view, HttpSession httpSession,
		...
		try {
			...
			PDFGenerator.generatePDFFromHTML(totalInformation,messageService,PDFGenerator.AUTOMATED,httpRequest.getSession().getId());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			view.setViewName("html/generic-error");
		
			e.printStackTrace();
		}
		
		emailService.sendEmailNotification(totalInformation.getPersonalDetails(),httpRequest.getSession().getId());
